...
[TGTR](#tgtr) | Получить данные о температуре в диапазоне
...
Получить данные температуры на запрошенном диапазоне индексов.

Предварительно должна быть вызвана команда [SSET](#sset).

**Запрос:**

Поле содержит [SIndexFilter](#sindexfilter): индексы начала и конце интересующего участка.

**Ответ:**

[STemperatureResult](#stemperatureresult): массив с данными температуры.
...
Массив данных температуры.

``` c++
    struct SIndexFilter
    {
        uint32              count;      ///< количество данных
        STemperatureIndex   items[];    ///< данные температуры
    };
```
...
Данные температуры, привязанные к индексу.

``` c++
    struct STemperatureIndex
        : SIndex
        , STemperature
    {
    };
